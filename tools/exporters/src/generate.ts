...
import { createLoomTraceArchive, type TraceBundle, type TraceFrame, type TraceFamily } from "@neuroloom/core";
...
async function main() {
  ...
  const bundles = [createMlpTrace(), createCnnTrace(), createTransformerTrace()];
...
function createMlpTrace(): TraceBundle {
    ...
    payloads.set(
      ...
      JSON.stringify({
        ...
        selectionDetails: {
          ...
          "hidden-c": detail("Hidden unit C", "Separates lower curve fragments.", [metric("activation", 0.47 + Math.cos(t * 4) * 0.12), metric("weight norm", 1.28)]),
          output: detail("Classifier output", "Aggregates the mixed features into a class logit.", [metric("logit", -0.12 + t * 1.44), metric("confidence", confidence)])
        }
      })
    );
  ...
  return {
    ...
    narrative: {
      ...
      chapters: [
        chapter("input-to-hidden", "Input Fan-Out", [0, 6], "hidden-a", "Forward pulses spread raw x/y features into the first hidden layer."),
        chapter("loss-anchor", "Loss Snapshot", [7, 10], "loss", "The scalar loss frame acts like a bright checkpoint before the backward pulse returns."),
        chapter("parameter-settle", "Parameter Settle", [11, 23], "output", "Update frames reveal a calmer, cleaner boundary and stronger class confidence.")
      ]
    },
    payloads
  };
  ...
  for (let frame = 0; frame < frameCount; frame += 1) {
    ...
    payloads.set(
      ...
      JSON.stringify({
        ...
        selectionDetails: {
          "conv-1": detail("Conv stage 1", "Early filters react to edge energy and cloth folds.", [metric("kernel energy", 0.72 - t * 0.1), metric("map entropy", 0.63)]),
          "conv-2": detail("Conv stage 2", "Later filters compress textures into category signatures.", [metric("kernel energy", 0.88 - t * 0.18), metric("map entropy", 0.44)]),
          dense: detail("Dense head", "Final pooled evidence is compressed into class scores.", [metric("activation", 0.56 + t * 0.18), metric("sparsity", 0.48)])
        }
      })
    );
    payloads.set(
      ...
      JSON.stringify({
        ...
        selectionDetails: {
          ...
          output: detail("Classifier output", "Class scores stabilize as texture evidence sharpens.", [metric("top-1", accuracy), metric("logit gap", 0.21 + t * 0.34)])
        }
      })
    );
  ...
  return {
    ...
    narrative: {
      ...
      chapters: [
        ...
        chapter("spatial-compression", "Spatial Compression", [7, 12], "pool-2", "Pooling collapses resolution while preserving salient activations."),
        chapter("class-evidence", "Class Evidence", [13, 19], "output", "The dense head aggregates compressed evidence into a stable garment class.")
      ]
    },
    payloads
  };
    ...
    payloads.set(
      ...
      JSON.stringify({
        ...
        selectionDetails: {
          ...
          logits: detail("Logits head", "Vocabulary scores sharpen as the next token becomes predictable.", [metric("entropy", entropy), metric("top-1 prob", confidence)]),
          decode: detail("Decode step", "The selected next token stabilizes at the tail end of the replay.", [metric("step", frame), metric("temperature", 0.9)])
        }
      })
    );
  ...
  return {
    ...
    narrative: {
      ...
      chapters: [
        ...
        chapter("attention-ribbons", "Attention Ribbons", [7, 15], "attn", "Attention heads brighten and tighten as decode confidence rises."),
...
function chapter(id: string, label: string, frameRange: [number, number], defaultSelection: string, description: string) {
  return { id, label, frameRange, defaultSelection, description };
}

function payloadEntry(id: string, kind: "render" | "inspect") {
...
function createAttentionMatrix(size: number, t: number) {
  return Array.from({ length: size }, (_, row) =>
    Array.from({ length: size }, (_, column) => {
      const base = 0.1 + Math.max(0, 0.9 - Math.abs(row - column) * 0.2);
